...
public class GenerateNodes : MonoBehaviour
{
    ...
    //        Node.ConnectionType nearestType = Node.ConnectionType.walkable;

    //        // In this list each candidate is a tuple holding three values
    //        List<(Node node, float dist, Node.ConnectionType type)> candidate = new();

    //        foreach(Node OtherEdgeNode in EdgeNodes)
    //        {
    //            if (OtherEdgeNode == edgeNode) continue;

    //            float distX = Mathf.Abs(edgeNode.transform.position.x - OtherEdgeNode.transform.position.x);
    //            float distY = OtherEdgeNode.transform.position.y - edgeNode.transform.position.y;
    ...
    //                nearestType = canJump ? Node.ConnectionType.jump : Node.ConnectionType.fall;
    //            }
    //        }
    ...
    //            {
    //                if (otherNode == edgeNode) continue;

    //                if (edgeNode.connections.Exists(c => c.targetNode == otherNode)) continue; // skip if already connected

    //                float distX = Mathf.Abs(edgeNode.transform.position.x - otherNode.transform.position.x);
    //                float distY = otherNode.transform.position.y - edgeNode.transform.position.y;

    //                float distance = Vector2.Distance(edgeNode.transform.position, otherNode.transform.position);

    //                // Check if otherNode is reachable by jump or fall
    //                bool canJump = distY > 0 && distY <= maxJumpHeight && distance <= maxJumpDistance;
    //                bool canFall = distY < 0 && Mathf.Abs(distY) <= maxFallHeight && distance <= maxFallDistance;
    ...
    //        if (nearestNode != null)
    //        {
    //            edgeNode.connections.Add(new Node.NodeConnection(nearestNode, nearestType));
    //            nearestNode.connections.Add(new Node.NodeConnection(edgeNode, nearestType));
    //            //Debug.DrawLine(edgeNode.transform.position , nearestNode.transform.position,Color.black ,Mathf.Infinity);
    //        }
    //    }
